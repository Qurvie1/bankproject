...
int main() {
  Bank bank;
  std::vector<std::string> data1{"rofl1", "rofl2", "rofl3", "rofl4", "rofl5", "rofl6", "rofl7"};
  std::vector<std::string> data2{"check1", "check2", "check3", "check4", "check5", "check6", "rofl8"};
  bank.CreateClient(data1);
  bank.CreateClient(data2);
  std::cout << bank.Clients() << '\n';
  bank.CreateDebitAccount(0);
  bank.CreateDebitAccount(1);
  bank.AddMoney(0, 5'000, 0);
  bank.AddMoney(1, 5'000, 0);
  bank.GetClient(0)->accounts[0]->Print();
  bank.GetClient(1)->accounts[0]->Print();
  auto transaction = bank.TransactionClient(0, 1, 0, 0, 3'000);
  bank.GetClient(0)->accounts[0]->Print();
  bank.GetClient(1)->accounts[0]->Print();
  return 0;
}